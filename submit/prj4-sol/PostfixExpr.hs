...
-- 
--evalIntExpr (Uminus e1) = - evalIntExpr e1

postfixExpr :: String -> PostfixExpr
helper :: [String] -> [PostfixExpr] -> PostfixExpr

postfixExpr postfix = helper (words postfix) []

helper [] (head : stack) = head

helper ("+" : tail) (first : second : stack) = helper tail (Add second first : stack)

helper ("-" : tail) (first : second : stack) = helper tail (Sub second first : stack)

helper ("*" : tail) (first : second : stack) = helper tail (Mul second first : stack)

helper ("uminus" : tail) (first : stack) = helper tail (Uminus first : stack)

helper (head : tail) stack = helper tail ((Leaf (read head)) : stack)
